...
#include "bucket_lock.cpp"
#include "logger.cpp"
#include "perf_mon.cpp"


BucketLocker client_locks;
...
void remove_client_locked(int epoll_fd, int client_fd) {
	if (client_fd >= 0 && client_fd < (int)clients.size() &&
		clients[client_fd]) {
...
void remove_client(int epoll_fd, int client_fd) {
	std::lock_guard<std::mutex> lock(client_locks.get_mutex(client_fd));
...
void handle_client_data(int epoll_fd, int fd, TimePoint now) {
	...
	{
		std::lock_guard<std::mutex> lock(client_locks.get_mutex(fd));
...
void cleaner_thread(int epoll_fd) {
	while (server_running) {
		...
		for (size_t b = 0; b < NUM_BUCKETS; ++b) {
			{
				std::lock_guard<std::mutex> lock(client_locks.shards[b].mtx);
				for (int i = b; i < static_cast<int>(clients.size());
					 i += NUM_BUCKETS) {
					if (clients[i] && now - clients[i]->last_activ >
