...
public:
    ...
    void print_report(std::string_view label) {
        ...
        double avg_lat = l_count > 0 ? (double)total_latency_ns / l_count / 1000.0 : 0;
        double max_lat = (double)max_latency_ns / 1000.0;

        double avg_conn = c_count > 0 ? (double)total_conn_duration_ns / c_count / 1e9 : 0;
        ...
        std::cout << std::format(
            ...
            label, 
            active_clients.load(), 
            total_sent.load(), total_received.load(), total_errors.load(),
            avg_lat, max_lat, 
            avg_conn, max_conn
        );
    }
