...
Вместо создания отдельного потока на каждое соединение (модель thread-per-connection), используется несолько потоков, которые управляют всеми событиями через `epoll` и глобальный мьютекс.

**Плюсы:**
*   Для доступа к данным клиентов используется глобальлный мьютекст, что обеспечивает синхронизацию.
...
*   Время подключения для сбора метрик
...
## 7. Изменение дизайна
Вместо однопотока используется разные потоки для i\o и для очистки и статистики (прирост 4х). Вместо map используется vector указателей (изначально 1024) память будет выделяться в куче вместо стека.  